...
public sealed interface RootedTree<T, L> permits InnerNode, TerminalNode {
    ...
    default Inner<T, L> toInner(InnerNode<T, L> newParent, List<RootedTree<T, L>> newChildren,
                                       Function<L, L> labelTransformation) {
        return switch (this) {
            case Leaf<T, L> (T container,L label,InnerNode<T, L> parent) ->
                    new Inner<> (container, labelTransformation.apply(label), parent, newChildren);
            case Inner<T, L> (T container,L label, InnerNode<T, L> parent, List<RootedTree<T, L>> children) ->
                    new Inner<> (container, labelTransformation.apply(label), parent, children);
            case Root<T, L> (T container,L label,List<RootedTree<T, L>> children) ->
                    new Inner<> (container, labelTransformation.apply(label), newParent, children);
        };
    }

    default Leaf<T, L> toLeaf (InnerNode<T, L> newParent, Function<L, L> labelTransformation) {
        return switch (this) {
            case Leaf<T, L> (T container,L label,InnerNode<T, L> parent) ->
                    new Leaf<> (container, labelTransformation.apply(label), parent);
            ...
            case Root<T, L> (T container,L label,List<RootedTree<T, L>> ignored) ->
                    new Leaf<> (container, labelTransformation.apply(label), newParent);
        };
    }

    default Root<T, L> toRoot(List<RootedTree<T, L>> newChildren, Function<L, L> labelTransformation) {
        return switch(this) {
            case Leaf<T, L> (T container,L label,InnerNode<T, L> parent) ->
                    new Root<> (container, labelTransformation.apply(label), newChildren);
            case Inner<T, L> (T container,L label, InnerNode<T, L> ignored, List<RootedTree<T, L>> children) ->
                    new Root<> (container, labelTransformation.apply(label), children);
            case Root<T, L> (T container,L label,List<RootedTree<T, L>> ignored) ->
                    new Root<> (container, labelTransformation.apply(label), newChildren);
        };
    }
